...
public class MemberController {
	...
	public String submit(@Valid MemberVO memberVO, BindingResult result,
		...
		try {
			...
			if(member!=null && member.getAuth()==1) {//정지회원 메시지 표시
				result.reject("noAuthority");//에러코드
			}else {
				result.reject("invalidIdOrPassword");//에러코드
			}
			return formLogin();
		}
	}
	...
	public void viewProfile(MemberVO memberVO, HttpServletRequest request, Model model) {
		if(memberVO==null || memberVO.getPhoto_name()==null) {
			//업로드한 프로필 사진 정보가 없을 때 기본 이미지 표시
			getBasicProfileImage(request, model);
		}else {//업로드한 이미지 존재 시 읽기
